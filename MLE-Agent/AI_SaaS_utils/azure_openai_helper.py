...
def get_azure_openai_instance(
    deployment_name: str, temperature: int, parallel_tool_calling: bool = None  # type: ignore
) -> AzureChatOpenAI:
    """
    ...
    if parallel_tool_calling is None:
        ...
        return AzureChatOpenAI(
            ...
            temperature=temperature,
            # openai_api_version="2024-08-01-preview",
            openai_api_version="2024-02-01",  # type: ignore
            azure_endpoint=os.getenv("AZURE_OPENAI_ENDPOINT"),
            openai_api_key=os.getenv("AZURE_OPENAI_API_KEY"),  # type: ignore
            parallel_tool_calls=parallel_tool_calling,  # type: ignore
        )
